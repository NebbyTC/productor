...
class Faktura:
	...
	def build_xlsx(self, include_vat=False):
		...
		def build_with_vat():
			...
			for nr_produktu, produkt in enumerate(zakupy):

				# Obliczanie stawek
				kwota_vat = produkt.cena*(produkt.vat/100)
				kwota_netto = produkt.cena*(produkt.vat/100)+produkt.cena

				# Wpisywanie do tabeli
				arkusz[f"A{wiersz}"] = nr_produktu + 1
				arkusz[f"B{wiersz}"] = produkt.kod
				wpisz_nazwe(wiersz, produkt.nazwa)
				arkusz[f"E{wiersz}"] = produkt.ilosc
				arkusz[f"F{wiersz}"] = "{0:.2f}".format(produkt.cena)
				arkusz[f"G{wiersz}"] = "{0:.2f}".format(produkt.vat)
				...
				# Sumowanie stawek
				suma_brutto += produkt.cena
				suma_netto += kwota_netto
				suma_vat += kwota_vat
				
				kolejny_wiersz(produkt.nazwa)
		...
		def build_without_vat():
			...
			for nr_produktu, produkt in enumerate(zakupy):
				...
				wpisz_nazwe(wiersz, produkt.nazwa)
				arkusz[f"E{wiersz}"] = produkt.ilosc
				arkusz[f"F{wiersz}"] = produkt.cena
				arkusz[f"G{wiersz}"] = produkt.uwagi
			...
			arkusz[f"E{wiersz+1}"] = "Razem:"
			arkusz[f"F{wiersz+1}"] = "{0:.2f}".format(produkt.cena)
	...
	def build_pdf(self, include_vat=False):
		...
		def build_with_vat():
			...
			for nr_produktu, produkt in enumerate(zakupy):
				...
				canvas.drawString(kolumny["B"], ystart-365-14*wiersz, str(produkt.kod))
				wpisz_nazwe(wiersz, produkt.nazwa)
				canvas.drawString(kolumny["D"], ystart-365-14*wiersz, str(produkt.ilosc))
				canvas.drawString(kolumny["E"], ystart-365-14*wiersz, "{0:.2f}".format(produkt.cena))
				canvas.drawString(kolumny["F"], ystart-365-14*wiersz, "{0:.2f}".format(produkt.vat))
		...
		def build_without_vat():
			...
			Generuje fakturę bez podatku vat.
			"""
			nonlocal canvas, wiersz, kolumny, suma_brutto
